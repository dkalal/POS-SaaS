...
class SignupForm(forms.Form):
    ...
    phone = forms.CharField(max_length=64, label="Phone number")
    ...
    terms = forms.BooleanField(label="I accept the terms and privacy policy")
    ...
    def clean_email(self):
        ...
        if get_user_model().objects.filter(email__iexact=email).exists():
            raise forms.ValidationError("An account already uses this email. Sign in or use another email.")
        return email
...
class InviteTeamForm(forms.Form):
    ...
    role = forms.ChoiceField(choices=TenantInvitation.Role.choices)
...
class MembershipRoleForm(forms.Form):
    role = forms.ChoiceField(choices=TenantMembership.Role.choices, label="Workspace role")
